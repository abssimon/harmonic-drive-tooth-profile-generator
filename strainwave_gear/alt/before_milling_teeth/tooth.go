...
// 
func FlexTooth(p Point, rotate float64) ([]Point, []Point) {

    tooth := BasicTooth(rotate) 

    points := []Point{}
    prodPoints := []Point{} // also pick tan points to rotate one flanc vertical and some center points for production 
    // append points in order
    c := tooth.C4.Coordinates()
    for i := len(c) - 1; i >= 0; i-- {
        points = append(points, Point{c[i].X + p.X, c[i].Y + p.Y})
    }
    prodPoints = append(prodPoints, Point{tooth.C4.X + p.X, tooth.C4.Y + p.Y})
    prodPoints = append(prodPoints, Point{c[0].X + p.X, c[0].Y + p.Y})
    ...
    prodPoints = append(prodPoints, Point{c[len(c) - 1].X + p.X, c[len(c) - 1].Y + p.Y}) 
    
    c = tooth.C1.Coordinates()
    for i := len(c) - 1; i >= 0; i-- {
        points = append(points, Point{c[i].X + p.X, c[i].Y + p.Y})
    }
    prodPoints = append(prodPoints, Point{tooth.C1.X + p.X, tooth.C1.Y + p.Y})
    prodPoints = append(prodPoints, Point{c[len(c) - 1].X + p.X, c[len(c) - 1].Y + p.Y})

    return points, prodPoints
}
