...
func main() {
    ...
    circle2 := newCircle(72, 436, 260)  // harmic shorter y-154
    circle3 := newCircle(-72, 436, 260) // harmic shorter y-154
    ...
    points = circle2.CoordinatesBetween(Point{circle2.X - 52, circle2.Y + 67}, l1.p1)
    for i := len(points) - 1; i >= 0; i-- {
        b.WriteString(fmt.Sprintf("%.14f %.14f ", moveX+points[i].X, moveY+points[i].Y))
    }

    _, l2 = circle3.InnerTangentTo(circle4)
    points = circle3.CoordinatesBetween(l2.p2, Point{circle3.X + 52, circle3.Y + 67})
    for i := len(points) - 1; i >= 0; i-- {
        b.WriteString(fmt.Sprintf("%.14f %.14f ", moveX+points[i].X, moveY+points[i].Y))
    }
